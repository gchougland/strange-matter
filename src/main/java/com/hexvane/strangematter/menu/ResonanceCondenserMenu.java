    ...
    protected void addPlayerSlots(Inventory playerInventory) {
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 9; ++col) {
                this.addSlot(new Slot(playerInventory, col + row * 9 + 9, 8 + col * 18, 68 + row * 18)); // Adjust these coordinates to match your texture
            }
        }
        
        // Add player hotbar - positioned to match your custom GUI texture
        for (int col = 0; col < 9; ++col) {
            this.addSlot(new Slot(playerInventory, col, 8 + col * 18, 124)); // Adjust these coordinates to match your texture
        }
    }
}
